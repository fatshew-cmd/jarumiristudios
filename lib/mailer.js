...
function sendBookingConfirmation(booking) {
  const body = `
    ...
    <p style="margin:0 0 16px;">${codeChip(booking.crCode)}</p>
    ...
    ${button("Track your project →", "https://jarumiristudios.com/track")}
...
function sendDepositExpiredEmail(booking) {
  const body = `
    <p style="margin:0 0 16px;">Hi ${booking.name},</p>
    <p style="margin:0 0 16px;">The deposit deadline for your project (${codeChip(booking.crCode)}) has passed without payment, so we've placed it on hold.</p>
...
function sendFinalExpiredEmail(booking) {
  const body = `
    <p style="margin:0 0 16px;">Hi ${booking.name},</p>
    <p style="margin:0 0 16px;">The due date for your final payment on project ${codeChip(booking.crCode)} has passed, so we've voided that invoice link for security.</p>
...
function sendDepositReminderEmail(booking) {
  ...
  const body = `
    <p style="margin:0 0 16px;">Hi ${booking.name},</p>
    <p style="margin:0 0 16px;">Just a reminder that the deposit for your project (${codeChip(booking.crCode)}) is due by <strong>${dueDateStr}</strong>. If it passes unpaid, the project will be put on hold.</p>
...
function sendFinalReminderEmail(booking) {
  ...
  const body = `
    <p style="margin:0 0 16px;">Hi ${booking.name},</p>
    <p style="margin:0 0 16px;">Just a reminder that the final payment for your project (${codeChip(booking.crCode)}) is due by <strong>${dueDateStr}</strong>. If it passes unpaid, the payment link will be voided.</p>
    <p style="margin:0;">You can pay from ${payElsewhere} using your BR code if you can't find the original invoice email.</p>
    ${signOff()}
  `;
  return sendMail({
