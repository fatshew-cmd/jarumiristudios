...
function sendAdminNudgeAlert(booking) {
  return sendMail({
    to: process.env.ADMIN_EMAIL,
    subject: `Client nudge — ${booking.crCode}`,
    html: `
      <p>A client is asking for an update on their project.</p>
      <ul>
        <li><strong>Project:</strong> ${booking.crCode}</li>
        <li><strong>Client:</strong> ${booking.name} (${booking.email})</li>
      </ul>
...
module.exports = { sendMail, sendBookingConfirmation, sendAdminNewBookingAlert, sendAcceptanceEmail, sendAdminInvoiceAlert, sendAdminPaymentAlert, sendAdminPauseAlert, sendAdminNudgeAlert, sendDepositExpiredEmail, sendAdminDepositExpiredAlert, sendFinalExpiredEmail, sendAdminFinalExpiredAlert, sendAdminUnexpectedPaymentAlert };
