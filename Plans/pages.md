...
| `GET /hire/success` | Post-submit | Shown to guests after submitting; offers inline account creation to track the booking going forward — the offer shows a live "Save $X on this project" preview (`signupDiscountPreview`, 2026-07-12, uncommitted) when the triggering booking is still eligible for the signup discount to apply retroactively (see `/signup`) |
| `POST /hire/coupon/validate` | — | AJAX coupon validation for the `/hire` form |
| `POST /signup` | — | Inline signup, embedded on both `/hire/success` and `/track` (2026-07-11) — creates a `User`, links the relevant booking via `crCode`, logs in; redirects back to whichever page it was submitted from via a whitelisted `returnTo` field. Grants a one-time 15%-off "welcome discount" (`User.discountPercent`/`discountExpiresAt`, 15-day window, `ce11320`) to genuinely first-time emails only (an email that's ever owned an account before, even a since-deleted one, doesn't get a second discount, `4db3d62`) — normally redeemable on a later `/hire` submission, but if the triggering booking hasn't been priced yet (`!agreedPrice`) the discount is applied to it directly instead (retroactive application, 2026-07-12, uncommitted), which is what actually makes it an acquisition incentive rather than a reward for some hypothetical next project. An hourly job (`lib/discountExpiry.js`) emails + in-app-notifies accounts whose unused discount is expiring within 3 days |
| `/track` | Project Tracker | Look up a booking by BR code or name + email combo; shows deposit due date notice while `depositStatus` is `pending`, estimated delivery date once admin sets it, and final deliverable downloads once `status === "completed"`. An unlinked booking (no `clientId`) also shows an account nudge (2026-07-11) — inline signup (with the same live signup-discount preview as `/hire/success`, 2026-07-12, uncommitted), or a "log in to link" prompt if an account already exists for that email |
