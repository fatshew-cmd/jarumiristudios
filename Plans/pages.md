...
| `GET /go/:crCode` | — | Smart link used behind every BR code chip in transactional emails (`codeChipLink()`, `lib/mailer.js`, 2026-07-11, `24a4127`) — sends a logged-in client to their own `/dashboard/booking/:id` if the code belongs to them (and isn't `filesDeleted`), otherwise falls back to `/track?code=`. Replaced plain `codeChip()`/hardcoded `/track` links across all mailer templates |
| `/login` | Client Login | Existing client login; supports `?next=` redirect and `?cr=` to link a just-submitted booking on login; links out to `/signup` for a new account (2026-07-14, `ebb3885` — previously linked to `/hire`); login errors render via the toast system instead of a static banner (2026-07-11, `24a4127`) |
...
| Route | Page | Purpose |
|-------|------|---------|
| `/dashboard` | Client Dashboard | All submitted requests, statuses, payment progress. Shows an amber obligation banner (with an inline form) when `user.pendingTestimonialObligation` is set — see `POST /dashboard/booking/:id/testimonial` below |
| `POST /dashboard/booking/:id/testimonial` | — | Clears a completed Free-tier booking's testimonial-or-gallery-rights obligation (`User.pendingTestimonialObligation`) — either a written testimonial (`BookingRequest.testimonial`) or checking "grant gallery rights" (`galleryRightsGranted`) satisfies it; blocks further Free-tier `/hire` submissions until cleared (2026-07-13) |
...
| `POST /admin/booking/:id/send-deposit` | — | Create/reuse Stripe customer, send 30% deposit invoice with an admin-set due date (`due_date`), flips status to `accepted`, sends acceptance email; blocked (redirects with an error) while `status === "pending"` — booking must reach `in-review` first so the client gets a window to upload files before seeing a payment ask. Also blocked until at least one uploaded file has a `video/*` mimetype (2026-07-11, `df24aea`) — no deposit ask against a project that's still just a form submission with no footage attached; the "Pay deposit now" link on `dashboard-booking.ejs` additionally guards clicks client-side with an explanatory toast for any pending invoice that predates this check (`68aa4f2`) |
