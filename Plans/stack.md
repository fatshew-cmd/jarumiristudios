...
- **Invoice expiry job** (`lib/invoiceExpiry.js`, renamed from `depositExpiry.js`) — in-process `setInterval`, runs hourly, no external cron dependency; three checks:
  ...
  - Stale-after-final-expiry (`ae4471e`): if a `finalPaymentStatus: "none"` booking's (now-stale) `finalDueDate` is more than 3 days in the past with no fresh final invoice sent since, the project is auto-archived (same archive-folder-move as the deposit case) and both client + admin are emailed.
